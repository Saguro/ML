...
class DataProvider:
    ...
    def get_video(self, video_name: str) -> pd.DataFrame:
        ...
        data_zip = ZipFile(self._zip_file_path)

        # stworzenie tablicy - odczytanie csv, nazywanie kolumn etc
        video_data = pd.read_csv(
            StringIO(str(data_zip.open(f"{self._annotations_zip}{video_name}.csv").read(), 'utf-8')),
            header=None,
        ...
        for i, photo_name in video_data['file_name'].iteritems():
            video_data.loc[i, 'photo'] = Image.open(BytesIO(data_zip.open(self._videos_zip + photo_name).read()))
        return video_data
    ...
    def get_types(self, stage) -> pd.DataFrame:
        data_zip = ZipFile(self._zip_file_path)

        read_files = data_zip.open(f'data/{stage}videos').read().split()
        video_data = []
